...
function initData(line) {
  data = line.trim().split(" ").map(value => Number(value));
}

function calcBreakEvenPoint() {
  const fixedValue = data[0];
  const originValue = data[1];
  const saleValue = data[2];

  const diff = saleValue - originValue;
  ...
  const result = Math.floor(fixedValue / diff) + 1;
  console.log(result);
}