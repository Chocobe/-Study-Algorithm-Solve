 ...
 * 
 *    * 여러분이 작성할 프로그램은 초기에 모든 방이 비어있다고 가정하에 이 정책에 따라 N 번째로 도착한 손님에게 배정될 방 번호를
 *      계산하는 프로그램이다. 첫 번째 손님은 101 호, 두 번째 손님은 201 호 등과 같이 배정한다. 
...
function initData(line) {
  const arr = line.trim().split(" ").map(value => Number(value));
  specList.push({
    height: arr[0],
    width: arr[1],
    num: arr[2],
...
function solution(spec) {
  const roomNum = Math.floor((spec.num - 1) / spec.height) + 1;
  const floorNum = (spec.num - 1) % spec.height + 1;

  console.log(`${floorNum}${roomNum < 10 ? "0".concat(roomNum.toString()) : roomNum}`)
}